...
echo 'starting DB..'
STARTED="yes"
docker-compose up -d couchbase >/dev/null 2>&1
waitForContainer couchbase
echo 'Init cluster...'
while true ; do
...
docker-compose up -d couchbase_dev >/dev/null 2>&1

echo "Open http://localhost:8091/ to manage"
echo "Ctrl+C to end"
sleep 2
docker-compose logs --follow couchbase_dev
